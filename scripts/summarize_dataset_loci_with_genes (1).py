...
def find_subset_file(cfg, dataset, table, chromosome):
    """Locate Step 3 subset file for dataset/table/chromosome."""
    candidates = []
    ...
    if cfg_results_dir:
        candidates.append(
            os.path.join(
                ...
                dataset,
                f"{table}_subset_dir",
                f"{dataset}_{table}_chr{chromosome}_subset.tsv",
            )
        )

    # Local workspace-relative fallback
    candidates.append(
        os.path.join(
            "results",
            "gwas_subset",
...
def load_subset_for_max_p(cfg, dataset, table, chromosome):
    """
    Load subset SNPs for max-p scan with normalized columns: POS, SNP_ID, PVAL.
    Returns DataFrame (possibly empty).
    """
    subset_path = find_subset_file(cfg, dataset, table, chromosome)
    if not subset_path:
        return pd.DataFrame(columns=["POS", "SNP_ID", "PVAL"])

    try:
        df = pd.read_csv(subset_path, sep="\t", dtype=str, low_memory=False)
    except Exception:
        return pd.DataFrame(columns=["POS", "SNP_ID", "PVAL"])
    ...
    pos_col = find_col(df.columns, [r"^POS$", r"^BP$", r"^position$", r"^pos$"])
    p_col = find_col(df.columns, [r"^P$", r"^p$", r"^p_", r"^pval", r"pvalue"])
    id_col = find_col(df.columns, [r"^ID$", r"^rsid$", r"^snpid$", r"^markername$", r"^varid$", r"^SNP$"])

    if not pos_col or not p_col or not id_col:
        return pd.DataFrame(columns=["POS", "SNP_ID", "PVAL"])

    out = df[[pos_col, id_col, p_col]].copy()
    out.columns = ["POS", "SNP_ID", "PVAL"]
    out["POS"] = pd.to_numeric(out["POS"], errors="coerce")
    out["PVAL"] = pd.to_numeric(out["PVAL"], errors="coerce")
    out = out.dropna(subset=["POS", "SNP_ID", "PVAL"])
...
def build_summary(merged_df, all_loci_df, assigned_codes, gwastables, cfg, chromosome=None):
    """
        Attach one column per dataset x table to merged_df:
            {dataset}__{table}__top_p  - minimum p-value in the locus
            {dataset}__{table}__top_snp_rsid - rsID of SNP with minimum p-value
            {dataset}__{table}__top_snp_p - p-value of that top SNP
            {dataset}__{table}__max_p - maximum p-value in the locus
            {dataset}__{table}__max_p_snp_rsid - rsID of SNP with maximum p-value
    ...
    for dataset_entry in gwastables:
        ...
        for table_entry in dataset_entry.get("tables", []):
            ...
            rsid_col = f"{dataset}__{table}__top_snp_rsid"
            top_snp_p_col = f"{dataset}__{table}__top_snp_p"
            max_p_col = f"{dataset}__{table}__max_p"
            max_p_rsid_col = f"{dataset}__{table}__max_p_snp_rsid"
            ...
            subset_snps = load_subset_for_max_p(cfg, dataset, table, chromosome) if chromosome else pd.DataFrame(columns=["POS", "SNP_ID", "PVAL"])

            if subset.empty:
                summary[p_col] = np.nan
                summary[rsid_col] = np.nan
                summary[top_snp_p_col] = np.nan
                summary[max_p_col] = np.nan
                summary[max_p_rsid_col] = np.nan
            else:
                ...
                summary[rsid_col] = summary["locus_code"].map(
                    lambda lc, b=best: b.loc[lc, "top_snp"] if (lc in b.index and "top_snp" in b.columns) else np.nan
                )
                # BUG-5 FIX: __top_snp_p must come from the subset SNP scan
                # (the p-value of the best SNP found by scanning Step-3 subset
                #  SNPs within the locus window), NOT from the locus-level top_p
                # which is the value already stored in p_col.  They are logically
                # different: p_col is the minimum p across all source-locus rows
                # that were merged into this locus, while top_snp_p_col is the
                # minimum per-SNP p-value from the dense subset scan.
                # We fill top_snp_p_col AFTER the subset_snps block below.

                # Compute maximum p-value SNP per merged locus from Step 3 subset SNPs
                if subset_snps.empty:
                    summary[top_snp_p_col] = np.nan   # no subset scan → fall back to NaN
                    summary[max_p_col] = np.nan
                    summary[max_p_rsid_col] = np.nan
                else:
                    min_p_vals  = []
                    ...
                    max_vals = []
                    max_rsids = []
                    for _, loc_row in summary.iterrows():
                        loc_start = int(loc_row["start"])
                        loc_end = int(loc_row["end"])
                        in_locus = subset_snps[(subset_snps["POS"] >= loc_start) & (subset_snps["POS"] <= loc_end)]
                        if in_locus.empty:
                            min_p_vals.append(np.nan)
                            min_p_rsids.append(np.nan)
                            max_vals.append(np.nan)
                            max_rsids.append(np.nan)
                        else:
                            # top SNP = minimum p in the subset scan
                            min_idx = in_locus["PVAL"].idxmin()
                            min_p_vals.append(float(in_locus.loc[min_idx, "PVAL"]))
                            min_p_rsids.append(str(in_locus.loc[min_idx, "SNP_ID"]))
                    summary[top_snp_p_col] = min_p_vals
                    # Overwrite rsid_col with the SNP from the subset scan
                    # (more precise than the source-locus top_snp which may be
                    #  a per-source-locus best, not a per-position best)
                    summary[rsid_col] = min_p_rsids
                    ...
                    for _, loc_row in summary.iterrows():
                        ...
                        if in_locus.empty:
                            ...
                            idx = in_locus["PVAL"].idxmax()
                            max_vals.append(float(in_locus.loc[idx, "PVAL"]))
                            max_rsids.append(str(in_locus.loc[idx, "SNP_ID"]))

                    summary[max_p_col] = max_vals
                    summary[max_p_rsid_col] = max_rsids

            highlight_cols.append(p_col)
            highlight_cols.append(top_snp_p_col)
            highlight_cols.append(max_p_col)
...
def main():
    ...
    if all_loci.empty:
        ...
        summary, highlight_cols = build_summary(merged, all_loci, [], gwastables, cfg, args.chromosome)
    ...
    summary, highlight_cols = build_summary(merged, all_loci, assigned, gwastables, cfg, args.chromosome)
